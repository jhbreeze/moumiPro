...
function sendOk() {
	...
	f.action = "${pageContext.request.contextPath}/manual/${mode}";
	f.submit();
}

<c:if test="${mode=='update'}">
function deleteFile(fileNum) {
	let url = "${pageContext.request.contextPath}/manual/deleteFile";
...
<div class="container">
	<div class="body-container">	
		...
		<div class="body-main">
			...
			<form name="manualForm" method="post" enctype="multipart/form-data">
				...
				<table class="table table-borderless">
 					<tr>
						<td class="text-center">
							...
							<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/manual/list';">${mode=='update'?'수정취소':'등록취소'}&nbsp;<i class="bi bi-x"></i></button>
						</td>
						<c:if test="${mode=='update'}">
							<input type="hidden" name="manualNum" value="${dto.manualNum}">
...
$(function(){
	...
	if( img ) {
		img = "${pageContext.request.contextPath}/uploads/manual/"+img;
	...
	$(".table-form .thumbnail-viewer").click(function(){
		$("form[name=manualForm] input[name=thumbnailFile]").trigger("click");
	});
	
	$("form[name=manualForm] input[name=thumbnailFile]").change(function(){
...
$(function(){
	...
	$("body").on("click", ".table-form .img-add", function(){
		$("form[name=manualForm] input[name=addFiles]").trigger("click");
	});
	
	$("form[name=manualForm] input[name=addFiles]").change(function(){
		if(! this.files) {
			let dt = new DataTransfer();
			for(let f of sel_files) {
				dt.items.add(f);
			}
			document.manualForm.addFiles.files = dt.files;
